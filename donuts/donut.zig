...
q(i)*(c(j)+2)*c(A)                - q(j)*q(A);var x:i32 =
@intFromFloat(40                     + 30*m*(c(i)*(c(j) + 2) 
* c(B)-t*q(B)));                      var y:i32=@intFromFloat(
12+15*m*(c(i)*(                       c(j)+2)*q(B)+t*c(B)));var
p:u32=@intCast(                       x+80*y);var N: i32 = 
@intFromFloat(8*(                    (q(j)*q(A)-q(i)*c(j) *
c(A))*c(B)-q(i)  *                  c(j)*q(A) - q(j)*c(A) -
c(i)*c(j)*q(B)));if               (22 > y and y > 0 and x > 0 
 and 80 > x and m > z[p]){z[p] = m;var index: usize = if 
